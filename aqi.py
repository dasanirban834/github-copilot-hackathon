...
def forecastedAQI(Latitude, Longitude, city, api_key):
    base_url = f"http://api.openweathermap.org/data/2.5/air_pollution/forecast?lat={Latitude}&lon={Longitude}&appid={api_key}"
    response = requests.get(base_url)
    aqi_data = json.loads(response.text)
    Timestamp = []
    Gas = []
    for i in range(1, len(aqi_data['list'])):
        timestamp_unix = aqi_data['list'][0]['dt']
        Timestamp.append(timestamp_unix)
        aqi = aqi_data['list'][0]['main']['aqi']
        co = int(aqi_data['list'][0]['components']['co'])
        no = int(aqi_data['list'][0]['components']['no'])
        no2 = int(aqi_data['list'][0]['components']['no2'])
